-- Sample Blog Content for PT Jaminan Solusi Bisnis
...
-- Note: In actual implementation, these would be the actual UUIDs generated

-- Sample blog posts
-- Note: Replace category_id with actual UUIDs from your database
...
-- Note: Replace post_id and tag_id with actual UUIDs

-- For Post 1 (Umroh Guide)
INSERT INTO blog_post_tags (post_id, tag_id)
SELECT
  (SELECT id FROM blog_posts WHERE slug = 'panduan-lengkap-persiapan-umroh-pertama-kali' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'umroh' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;

INSERT INTO blog_post_tags (post_id, tag_id)
SELECT
  (SELECT id FROM blog_posts WHERE slug = 'panduan-lengkap-persiapan-umroh-pertama-kali' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'persiapan' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;

INSERT INTO blog_post_tags (post_id, tag_id)
SELECT
  (SELECT id FROM blog_posts WHERE slug = 'panduan-lengkap-persiapan-umroh-pertama-kali' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'panduan' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;

-- For Post 2 (Packing Tips)
INSERT INTO blog_post_tags (post_id, tag_id)
SELECT
  (SELECT id FROM blog_posts WHERE slug = '10-tips-packing-efisien-traveling-jarak-jauh' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'tips' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;

INSERT INTO blog_post_tags (post_id, tag_id)
SELECT
  (SELECT id FROM blog_posts WHERE slug = '10-tips-packing-efisien-traveling-jarak-jauh' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'travel' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;

INSERT INTO blog_post_tags (post_id, tag_id)
SELECT
  (SELECT id FROM blog_posts WHERE slug = '10-tips-packing-efisien-traveling-jarak-jauh' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'backpacker' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;

-- For Post 3 (Halal Tourism)
INSERT INTO blog_post_tags (post_id, tag_id)
SELECT
  (SELECT id FROM blog_posts WHERE slug = '5-destinasi-wisata-halal-terbaik-indonesia' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'wisata-halal' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;
...
-- For Post 4 (Hotel Guide)
INSERT INTO blog_post_tags (post_id, tag_id)
SELECT
  (SELECT id FROM blog_posts WHERE slug = 'tips-memilih-hotel-sesuai-budget-nyaman' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'hotel' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;
...
-- For Post 5 (Visa Guide)
INSERT INTO blog_post_tags (post_id, tag_id)
SELECT
  (SELECT id FROM blog_posts WHERE slug = 'panduan-lengkap-visa-schengen-wni' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'visa' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;
...
SELECT
  (SELECT id FROM blog_posts WHERE slug = 'panduan-lengkap-visa-schengen-wni' LIMIT 1),
  (SELECT id FROM blog_tags WHERE slug = 'dokumen' LIMIT 1)
ON CONFLICT (post_id, tag_id) DO NOTHING;

-- Note: Post counts will be calculated dynamically by the application
-- No need to update manually as the system handles it automatically